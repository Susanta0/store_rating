...
const SystemAdministrator = () => {
  ...
  useEffect(() => {
    ...
    const fetchDashboard = async () => {
      try {
        ...
        // Only show alert if there's a real error, not on first load
  ...
  const handleStoreSubmit = async (e) => {
    ...
    try {
      ...
      toast.success(`Store "${storeForm.name}" added successfully!`);
        ...
        <form onSubmit={handleStoreSubmit} className="space-y-4 max-w-lg">
          ...
          <div>
            ...
            <input
              ...
              value={storeForm.store_owner_id}
              onChange={(e) => setStoreForm({ ...storeForm, store_owner_id: e.target.value })}
              className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
            />
          </div>
